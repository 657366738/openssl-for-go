...
int X_BN_hex2bn_N(RSA*rsa,const char*in){

	return BN_hex2bn(&rsa->n,(const char*)in);
}
int X_BN_hex2bn_E(RSA*rsa,const char*in){

	return BN_hex2bn(&rsa->e,(const char*)in);
}
int X_BN_bn2bin_N(RSA*rsa,unsigned char*in){

	return BN_bn2bin(rsa->n,(unsigned char*)in);
}
int X_BN_bn2bin_E(RSA*rsa,unsigned char*in){

	return BN_bn2bin(rsa->e,(unsigned char*)in);
}
RSA* X_PEM_read_bio_RSAPrivateKey(BIO*bp,RSA**rsa){
	return PEM_read_bio_RSAPrivateKey(bp,rsa,NULL,NULL);
}
RSA* X_PEM_read_bio_RSAPublicKey(BIO*bp,RSA**rsa){

	return PEM_read_bio_RSA_PUBKEY(bp,rsa,NULL,NULL);
}